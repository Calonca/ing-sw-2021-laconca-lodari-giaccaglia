...
public class MiddleResourceMarketCLI extends ResourceMarketViewBuilder implements CLIBuilder {
    ...
    public void run() {
        ...
        if (Objects.requireNonNull(getThisPlayerCache()).getCurrentState().equals(CHOOSING_POSITION_FOR_RESOURCES.name())){
            choosePositions();
            return;
        }
    ...
    public void chooseMarbleConversion() {
        ...
        ActiveLeaderBonusInfo marketBonuses = Objects.requireNonNull(getThisPlayerCache()).getElem(ActiveLeaderBonusInfo.class).orElseThrow();
